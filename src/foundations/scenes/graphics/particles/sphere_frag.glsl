...
{ 
   ...
   vec3 f_specular = f_mat.specular.xyz * f_light.specular.xyz * pow(max(cosPhi, 0.0), f_mat.shininess * 4.0) * 10.0;

   fo_frag_color = f_frag_color * vec4((f_ambient + f_diffuse + f_specular), 1.0);
}
