...
void main()
{
    ...
    vec3 f_n_fres = normalize(fo_normal);
    float f_cos_fres = dot(f_V, f_n_fres);
    float f_fresnel = acos(f_cos_fres);
    f_fresnel = pow(clamp(f_fresnel - 0.3, 0.0, 1.0), 3);
;
    ...
    vec4 f_surface_color = mix(f_refraction_color, f_reflection_color, f_fresnel);
