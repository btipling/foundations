...
vec4 f_blinn_phong_lighting(Material f_mat, Light f_lights[10], uint num_lights, vec4 f_ambient_light) {
    ...
    do {
        ...
        float f_can_get_light_from_z_pos_0 = dot(f_L, vec3(0, 0, 1));
        float f_can_get_light_from_y_neg_1 = dot(f_L, vec3(0, -1, 0));
        float f_can_get_light_from_z_neg_2 = dot(f_L, vec3(0, 0, -1));
        float f_can_get_light_from_y_pos_3 = dot(f_L, vec3(0, 1, 0));
        float f_can_get_light_from_x_pos_4 = dot(f_L, vec3(1, 0, 0));
        float f_can_get_light_from_x_neg_5 = dot(f_L, vec3(-1, 0, 0));
