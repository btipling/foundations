...
pub fn init(allocator: std.mem.Allocator) *Marble {
    var sp = allocator.create(Marble) catch @panic("OOM");
    sp.* = .{
...
pub fn deinit(self: *Marble, allocator: std.mem.Allocator) void {
...
pub fn fillData(self: *Marble) void {
    ...
    for (0..self.dims) |h| {
        ...
        for (0..self.dims) |d| {
            ...
            for (0..self.dims) |w| {
                ...
                const dims_values: f32 = self.noise_3d.turbulence(h_f, d_f, w_f) / dims_f;
